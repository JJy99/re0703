...
public class CustomerManager3 {
   ...
   public static void main(String[] args) {
     ...
     public static void insertCustomerData() {
         ...
         for(Customer check : custList) {
         
            
            if(check.getEmail().equals(email)) {
               System.out.print("이메일이 중복됩니다. 새로운 이메일을 입력해주세요. \n 이메일 : ");   
               email = scan.next();// 갱신이 되는지 확인이 필요함
            }
   ...
   public static void deleteCustomerData() {
      ...
      boolean isEmpty = custList.isEmpty();
      
      if(isEmpty)
         System.out.println("list 가 비어있습니다. ");
      
      
      
      else
      {
      System.out.println("삭제를 원하는 이메일 : " );
      String delete_email = scan.next();
      
      
      
      for(int i=0;i<custList.size();i++)
      {
         if(custList.get(i).getEmail().equals(delete_email))
         {
            custList.remove(i);
